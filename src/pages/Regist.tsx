...
export default function Regist() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const response = await fetch('https://admin.expasign-edutime.site/api/team-regist/handle', {
                method: 'POST',
                headers: {
                    ...
                    {!selectedCompe ? (
                        ...
                        <form onSubmit={handleSubmit} noValidate>
                            <div className="mb-8 rounded-lg border border-blue-400/50 p-6">
                                ...
                                <div>
                                    <label htmlFor="team_name" className="mb-2 block text-sm font-bold text-gray-300">
                                        Nama Tim
                                    </label>
                                    <input
                                        type="text"
...
<div className="group">
    <label className="mb-2 block text-xs text-gray-300">Screenshot follow instagram @expasign</label>
    <div className="relative">
        <input
            ...
            id={`followExpa-${index}`}
            onChange={(e) => {
                console.log('File selected for followExpa:', e.target.files?.[0]?.name); // Debug log
                handleMemberChange(index, e);
                const filename = e.target.files?.[0]?.name || 'No file chosen';
                const display = document.getElementById(`followExpa-${index}-filename`);
        ...
        <div 
            ...
            onClick={() => {
                document.getElementById(`followExpa-${index}`)?.click();
            }}
        >
            <div className="flex items-center space-x-3">
                ...
                <div className="flex-1">
                    <p className="text-sm text-gray-300">Choose file</p>
                    <p className="text-xs text-gray-500" id={`followExpa-${index}-filename`}>
                        No file chosen
                    </p>
                </div>
    ...
    {getError(`members.${index}.followExpa`)}
                                        ...
                                        <div className="group">
                                            <label className="mb-2 block text-xs text-gray-300">Screenshot follow instagram @edutime</label>
                                            <div className="relative">
                                                <input
                                                    type="file"
                                                    name="followEdu"
                                                    onChange={(e) => {
                                                        handleMemberChange(index, e);
                                                        const filename = e.target.files?.[0]?.name || 'No file chosen';
                                                        const display = document.getElementById(`followEdu-${index}-filename`);
                                                        if (display) display.textContent = filename;
                                                    }}
                                                ...
                                                <div className="cursor-pointer rounded-lg border border-gray-600 bg-gray-700/50 p-4 transition-all duration-200 hover:border-blue-500 hover:bg-gray-700/70">
                                                    <div className="flex items-center space-x-3">
                                                        ...
                                                        <div className="flex-1">
                                                            <p className="text-sm text-gray-300">Choose file</p>
                                                            <p className="text-xs text-gray-500" id={`followEdu-${index}-filename`}>
                                                                No file chosen
                                                            </p>
                                                        </div>
                                                        <svg className="h-4 w-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        <div className="group">
                                            <label className="mb-2 block text-xs text-gray-300">Screenshot follow instagram @marsproject</label>
                                            <div className="relative">
                                                <input
                                                    type="file"
                                                    name="followMp"
                                                    onChange={(e) => {
                                                        handleMemberChange(index, e);
                                                        const filename = e.target.files?.[0]?.name || 'No file chosen';
                                                        const display = document.getElementById(`followMp-${index}-filename`);
                                                        if (display) display.textContent = filename;
                                                    }}
                                                    className="absolute inset-0 z-10 h-full w-full cursor-pointer opacity-0"
                                                    accept="image/*"
                                                ...
                                                <div className="cursor-pointer rounded-lg border border-gray-600 bg-gray-700/50 p-4 transition-all duration-200 hover:border-blue-500 hover:bg-gray-700/70">
                                                    <div className="flex items-center space-x-3">
                                                        ...
                                                        <div className="flex-1">
                                                            <p className="text-sm text-gray-300">Choose file</p>
                                                            <p className="text-xs text-gray-500" id={`followMp-${index}-filename`}>
                                                                No file chosen
                                                            </p>
                                                        </div>
                                                        <svg className="h-4 w-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        <div className="group">
                                            <label className="mb-2 block text-xs text-gray-300">Screenshot Repost Story</label>
                                            <div className="relative">
                                                <input
                                                    type="file"
                                                    name="repostSg"
                                                    onChange={(e) => {
                                                        handleMemberChange(index, e);
                                                        const filename = e.target.files?.[0]?.name || 'No file chosen';
                                                        const display = document.getElementById(`repostSg-${index}-filename`);
                                                        if (display) display.textContent = filename;
                                                    }}
                                                    className="absolute inset-0 z-10 h-full w-full cursor-pointer opacity-0"
                                                    accept="image/*"
                                                ...
                                                <div className="cursor-pointer rounded-lg border border-gray-600 bg-gray-700/50 p-4 transition-all duration-200 hover:border-blue-500 hover:bg-gray-700/70">
                                                    <div className="flex items-center space-x-3">
                                                        ...
                                                        <div className="flex-1">
                                                            <p className="text-sm text-gray-300">Choose file</p>
                                                            <p className="text-xs text-gray-500" id={`repostSg-${index}-filename`}>
                                                                No file chosen
                                                            </p>
                                                        </div>
                                                        <svg className="h-4 w-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <div className="mt-6">
                                ...
                                <div className="flex items-center gap-2">
                                    <input
                                        ...
                                        id="auto"
                                        value="auto"
                                        checked={teamData.payment_method === 'auto'}
                                        onChange={handleTeamChange}
                                    />
                                ...
                                <div className="flex items-center gap-2">
                                    <input
                                        ...
                                        id="transfer"
                                        value="transfer"
                                        checked={teamData.payment_method === 'transfer'}
                                        onChange={handleTeamChange}
                                    />
                                ...
                                {teamData.payment_method === 'transfer' && (
                                    <div className="mt-4">
                                        <label htmlFor="receipt" className="mb-2 block text-xs text-gray-300">
                                        ...
                                        <div className="relative">
                                            <input
                                                ...
                                                onChange={(e) => {
                                                    handleTeamChange(e);
                                                    const filename = e.target.files?.[0]?.name || 'No file chosen';
                                                    const display = document.getElementById('receipt-filename');
                                                    if (display) display.textContent = filename;
                                                }}
                                                className="absolute inset-0 z-10 h-full w-full cursor-pointer opacity-0"
                                                accept="image/*"
                                            ...
                                            <div className="cursor-pointer rounded-lg border border-gray-600 bg-gray-700/50 p-4 transition-all duration-200 hover:border-blue-500 hover:bg-gray-700/70">
                                                <div className="flex items-center space-x-3">
                                                    ...
                                                    <div className="flex-1">
                                                        <p className="text-sm text-gray-300">Choose payment receipt</p>
                                                        <p className="text-xs text-gray-500" id="receipt-filename">
                                                            Image file
                                                        </p>
                                                    </div>
                                                    <svg className="h-4 w-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
